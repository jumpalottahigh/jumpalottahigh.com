...
export async function generateStaticParams() {
  const slugs = await client
    .fetch<{ slug: string }[]>(allCategorySlugsQuery)
    .catch(() => [])
...
export default async function CategoryPage({
  ...
  const [posts, categoryData] = await Promise.all([
    ...
    client
      .fetch<Category | null>(categoryBySlugQuery, { slug: category })
      .catch(() => null)
  ])
  ...
  const title =
    categoryData?.title || category.charAt(0).toUpperCase() + category.slice(1)
