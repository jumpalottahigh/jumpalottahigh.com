...
const components: PortableTextComponents = {
  ...
  block: {
    h1: ({ children }) => (
      <h1 className='text-3xl font-black text-white mt-10 mb-4'>{children}</h1>
    ),
    h2: ({ children }) => (
  ...
  list: {
    bullet: ({ children }) => (
      <ul className='list-disc list-inside space-y-1 mb-4 text-[var(--foreground)]'>
        {children}
      </ul>
    ),
    number: ({ children }) => (
      <ol className='list-decimal list-inside space-y-1 mb-4 text-[var(--foreground)]'>
        {children}
      </ol>
    )
  }
